...
class RotateAttentionLayer(AttentionLayer):
    ...
    def __init__(
        self, attention, d_model, n_heads, d_keys=None, d_values=None, event_dispatcher=""
    ):
...
class MoLEncoder(nn.Module):
    ...
    def __init__(self, config, n_vocab):
        ...
        builder = RotateEncoderBuilder.from_kwargs(
            ...
            attention_type="linear",  # unless we do deterministic_eval here, we will have random outputs
            feature_map=partial(
                GeneralizedRandomFeatures, n_dims=config["num_feats"], deterministic_eval=True
            ),
            activation="gelu",
        )
...
class Smi_ted(nn.Module):
    ...
    def load_checkpoint(self, ckpt_path):
        ...
        self.net = Net(
            self.n_embd,
            n_output=self.config["n_output"] if "n_output" in self.config else 1,
            dropout=self.config["d_dropout"],
        )
    ...
    def extract_all(self, smiles):
        ...
        null_idx = smiles[
            smiles.isnull()
        ].index.to_list()  # keep track of SMILES that cannot normalize
        ...
        )  # DO NOT USE THIS FOR DOWNSTREAM TASKS, USE `pred_set` INSTEAD

        # add padding
        mask_embeddings = token_embeddings * input_mask_expanded
        token_embeddings = F.pad(
            mask_embeddings, pad=(0, 0, 0, self.max_len - mask_embeddings.shape[1]), value=0
        )
        ...
        for idx in null_idx:
            ...
            true_cte.insert(
                idx, np.array([np.nan] * (self.config["max_len"] * self.config["n_embd"]))
            )
            pred_cte.insert(
                idx, np.array([np.nan] * (self.config["max_len"] * self.config["n_embd"]))
            )
    ...
    def extract_embeddings(self, smiles):
        ...
        smiles_embeddings = self.decoder.autoencoder.encoder(
            token_embeddings.view(-1, self.max_len * self.n_embd)
        )
    ...
    def encode(self, smiles, useCuda=False, batch_size=100, return_torch=False):
        ...
        # process in batches
        n_split = (
            smiles.shape[0] // batch_size if smiles.shape[0] >= batch_size else smiles.shape[0]
        )
        embeddings = [
...
def load_smi_ted(vocab_filename="bert_vocab_curated.txt"):
    vocabulary_path = get_path_from_root("fm4m.models.smi_ted.smi_ted_light", vocab_filename)
    tokenizer = MolTranBertTokenizer(vocabulary_path)
