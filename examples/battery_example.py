...
train_emb = [
    np.nan if row.isna().all() else row.dropna().tolist() for _, row in train_emb.iterrows()
]
...
def compute_components(df, smi_cols, conc_cols):
    components = [
        df[smi].apply(pd.Series).mul(df[conc], axis=0) for smi, conc in zip(smi_cols, conc_cols)
    ]
